...
static int  	pack_chunk	(LONG *, int, int, int);
...
int		isum=0,iii=0;
/*
extern int	mar_get_status_block();
*/
printf("xxxx x=%d %d %d    pos=%d\n",x,y,fdesc,lseek( fdesc, 0, SEEK_CUR) );

	while(done < (x * y)) {

	    isum += (unsigned short)img[nc];
	    if ( (unsigned short)img[nc] > 0 ) iii++;
	  
	    end = diff_words(img, x, y, buffer, done);
	    done += (end - buffer) + 1;

	    /* Update status after some blocks */
	    nc++;
/*
	    if ( fmod( (double)nc, 10.0 ) == 0.0 )
	    	i = mar_get_status_block();
*/
