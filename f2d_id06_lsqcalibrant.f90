...
!  History:
!    03-Dec-2014: V0.5 Set variable parrametrers array to correct size 
!      (Hammersley)
!    30-Oct-2014: V0.4 Add "detector_offset" parameter and allow beam centre to
!      be different from rotation centre (Hammersley)
!    01-Oct-2014: V0.3 Testing (Hammersley)
!    10-Sep-2014: V0.2 Changes (Hammersley)
...
!      offset from the rotation axis to start of the detector is to be refined
     Logical, Intent(IN) :: refine_beam_centre ! .True., if the beam centre X
!      position is to be refined
     Logical, Intent(IN) :: refine_detector_distance ! .True., if the sample
...
!      array
     Integer, Parameter :: Max_variables = 7 ! Dimension for parameter arrays
!  Local Variables:
...
!     Define model parameters
        num_variables = 7
        PARAMS(1) = experiment%detector_offset
        PARAMS(2) = experiment%x_beam * experiment%x_pixel_size
        PARAMS(3) = experiment%y_beam * experiment%y_pixel_size
        PARAMS(4) = experiment%detector_distance
        PARAMS(5) = experiment%wavelength
        PARAMS(6) = experiment%tilt_plane_rotation
        PARAMS(7) = experiment%tilt_angle
 
!     Set scale factor of typical parameter changes
        SCALE_FACTORS(1) = 0.001 ! Detector offset poorly known (metres)
        SCALE_FACTORS(2) = 0.0001 ! X-beam should be small
        SCALE_FACTORS(3) = 0.0001 ! Y-beam should be small
        SCALE_FACTORS(4) = 0.005
        SCALE_FACTORS(5) = 5.0e-13
        SCALE_FACTORS(6) = 0.05
        SCALE_FACTORS(7) = 0.01
 
!     Set constrained parameters
        CONSTRAINTS(1) = .Not. refine_detector_offset
        CONSTRAINTS(2) = .Not. refine_beam_centre
        CONSTRAINTS(3) = .Not. refine_beam_centre
        CONSTRAINTS(4) = .Not. refine_detector_distance
        CONSTRAINTS(5) = .Not. refine_wavelength
        CONSTRAINTS(6) = .Not. refine_tilt
        CONSTRAINTS(7) = .Not. refine_tilt
...
!     Set output variables
        experiment%detector_offset = PARAMS(1)
        experiment%x_beam = PARAMS(2) / experiment%x_pixel_size
        experiment%y_beam = PARAMS(3) / experiment%x_pixel_size
        experiment%detector_distance = PARAMS(4)
        experiment%wavelength = PARAMS(5)
        experiment%tilt_plane_rotation = PARAMS(6)
        experiment%tilt_angle = PARAMS(7)

!     Reduce tilt rotation angle to -180.0 to 180.0 range
        Do While (experiment%tilt_plane_rotation .Gt. Pi)
           experiment%tilt_plane_rotation = &
             experiment%tilt_plane_rotation - 2 * Pi
        End Do
        Do While (experiment%tilt_plane_rotation .Lt. -Pi)
           experiment%tilt_plane_rotation = &
             experiment%tilt_plane_rotation + 2 * Pi
        End Do
