...
!+ F2D_FINDLLQUAD - Fit2D: FIND Lower left QUADrant of peak positions
...
!  Write (*, '(''Entered F2D_FINDLLQUAD'')')
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
 
!  Check status.
     If (status .Ne. St_goodvalue) Then
        Call ST_SAVE ('Subroutine F2D_FINDLLQUAD ' // Version)
        Return
     End If
...
!     Search upper-left quadrant
        ...
        x_vector = X_2DPEAKS(xmax_peaks, 1) - X_2DPEAKS(1, 1)
        y_vector = Y_2DPEAKS(xmax_peaks, 1) - Y_2DPEAKS(1, 1)
...
!        Calculate estimated position of peak from previously found peaks
           If (X_2DPEAKS(xpeak, Wrap_y(ypeak + 1)) .Gt. -1.7e38 .And. &
             X_2DPEAKS(Wrap_x(xpeak + 1), Wrap_y(ypeak + 1)) .Gt. -1.7e38 &
             .And. X_2DPEAKS(Wrap_x(xpeak + 1), ypeak) .Gt. -1.7e38) Then
 
!           The three corner peaks have been found, so vector
!           can be calculated
              x_coordinate = X_2DPEAKS(Wrap_x(xpeak + 1), ypeak) + &
                (X_2DPEAKS(xpeak, Wrap_y(ypeak + 1)) - X_2DPEAKS(Wrap_x(xpeak &
                + 1), Wrap_y(ypeak + 1)))
              y_coordinate = Y_2DPEAKS(Wrap_x(xpeak + 1), ypeak) + &
                (Y_2DPEAKS(xpeak, Wrap_y(ypeak + 1)) - Y_2DPEAKS(Wrap_x(xpeak &
                + 1), Wrap_y(ypeak + 1)))
...
!           First conduct search to nearest pixel over defined region, followed
!           by a sub-pixel search
