...
!  ***************************
!  *                         *
!  * f2d_id06_calibrant2.f90 *
!  *                         *
!  ***************************
 
!+ F2D_ID06_CALIBRANT2 -  CALIBRANT fit of wavelength / distance / etc.
     Subroutine F2D_ID06_CALIBRANT2 (gui, xmaxdat, ymaxdat, xnumdat, ynumdat, &
...
!  History:
!    20-Jun-2014: V0.2 Allow complete freedom (Hammersley)
!    17-Jun-2014: V0.1 Original, based on "F2D_ID06_CALIBRANT" (Hammersley)
!  Type Definitions:
     Implicit None
     Include 'io.inc'
!  Global Constants:
...
!  Check input status
     If (status .Ne. St_goodvalue) Then
        Call ST_SAVE ('Subroutine F2D_ID06_CALIBRANT2 ' // Version)
        Return
     End If
...
!        Write (*, '(''Entered F2D_ID06_CALIBRANT2'')')
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
...
!     choose number of output bins and control various other options
        ...
        LOGICALS(11) = refine_detector_distance
...
!     Set resulting values
        ...
        refine_detector_distance = LOGICALS(11)
...
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
!        Write (*, '(''F2D_ID06_CALIBRANT2: After GS_2DIMAGE'')')
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
...
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
!        Write (*, '(''F2D_ID06_CALIBRANT2: After GS_INPS_FCOORDINATES'')')
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
...
!        Estimate position of beam-centre in X-direction
           ...
           Write (message, &
             '(''INFO: Beam centre from 2 reflections = '', 1pe14.5)') &
             beam_centre
           Call IO_WRITE (message, status)
...
!     Search up to first 2 Bragg rings
        ...
        Call IO_WRITE ('INFO: Fitting first 2 Bragg Reflections', status)
...
!     Refine tilt to first two diffraction rings
        Call F2D_FITCALIBRANT (full_info, F2d_max_rings, num_rings, &
          ...
          refine_xbeam_centre, .False., refine_detector_distance, .False., &
          refine_tilt, experiment, radial_error, status)

!     Draw positions of un-tilted lines on image
        Call F2D_DRAW_ID06_LINES (experiment, Max_angles, num_rings, &
          D_SPACINGS, status)
...
!     Search up to first 4 Bragg rings
        Call IO_WRITE (' ', status)
        Call IO_WRITE ('INFO: Fitting first 4 Bragg Reflections', status)

        half_search_distance = Abs(x2 - x1) / 5.0
...
!     Search up to first 7 Bragg rings
        Call IO_WRITE (' ', status)
        Call IO_WRITE ('INFO: Fitting first 7 Bragg Reflections', status)
...
!     Search up to first 15 Bragg rings
        Call IO_WRITE (' ', status)
        Call IO_WRITE (&
          'INFO: Fitting first 15 Bragg Reflections (if available', status)

        half_search_distance = Abs(x2 - x1) / 7.0
...
!     Draw positions of un-tilted lines on image
        ...
        refine_detector_distance = save_refine_detector_distance

     End If
 
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
!     Write (*, '(''F2D_ID06_CALIBRANT2: End'')')
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG

     End Subroutine F2D_ID06_CALIBRANT2
!********1*********2*********3*********4*********5*********6*********7*********8
