...
!  ***************************
!  *                         *
!  * f2d_id06_calibrant3.f90 *
!  *                         *
!  ***************************
 
!+ F2D_ID06_CALIBRANT3 -  CALIBRANT fit of wavelength / distance / etc.
...
!    15-Sep-2014: V0.2 Add "refine_wavelength" (Hammersley)
...
!      array
     Integer, Parameter :: Max_choices = 25 ! Maximum number of choices
!      (need 5 extra spaces)
...
!      information is to be produced
     Logical, Save :: refine_detector_offset = .True. ! .True., if the detector
!      offset is to be refined
...
!      polarisation factor is to be refined
     Logical, Save :: refine_detector_distance = .True. ! .True., if the
...
!  Check input status
     If (status .Ne. St_goodvalue) Then
        Call ST_SAVE ('Subroutine F2D_ID06_CALIBRANT3 ' // Version)
        Return
     End If
...
!        Write (*, '(''Entered F2D_ID06_CALIBRANT3'')')
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
 
!     Transfer current beam centre
        x_ellipse = experiment%x_beam * experiment%x_pixel_size
        y_ellipse = experiment%y_beam * experiment%x_pixel_size
...
!        Try to input default parameters from the data-base
           ...
           Call IO_INQ_LKEYVALUE ('CALI_FULL_INFO', full_info, retstat, &
             status)
           Call IO_INQ_LKEYVALUE ('CALI_REFINE_OFFSET', refine_detector_offset, &
             retstat, status)
           Call IO_INQ_LKEYVALUE ('CALI_REFINE_BEAM', refine_beam_centre, &
             retstat, status)
           Call IO_INQ_LKEYVALUE ('CALI_REFINE_DISTANCE', &
             refine_detector_distance, retstat, status)
...
!     choose number of output bins and control various other options
        num_choices = 18 !!!! DISABLE "REFINE POLARISATION" AT PRESENT
        PROMPT(1) = 'CALIBRANT PATTERN REFINEMENT'
        PROMPT(2) = 'OF DISTANCE WAVELENGTH ETC.'
        BUTTONS(1) = 'DISTANCE'
        BUTTONS(2) = 'DETECTOR OFFSET'
        BUTTONS(3) = 'BEAM X'
        BUTTONS(4) = 'BEAM Y'
        BUTTONS(5) = 'WAVELENGTH'
        BUTTONS(6) = 'X-PIXEL SIZE'
        BUTTONS(7) = 'Y-PIXEL SIZE'
        BUTTONS(8) = 'ANGULAR SECTIONS'
        BUTTONS(9) = 'REJECT OUTLIERS'
        BUTTONS(10) = 'REJECT LIMIT'
        BUTTONS(11) = 'FULL INFO'
        BUTTONS(12) = 'REFINE OFFSET'
        BUTTONS(13) = 'REFINE BEAM CENTRE'
        BUTTONS(14) = 'REFINE DISTANCE'
        BUTTONS(15) = 'REFINE WAVELENGTH'
        BUTTONS(16) = 'REFINE TILT'
        BUTTONS(17) = 'EXTRA ITERATIONS'
        BUTTONS(18) = 'CORRECT PARALLAX'
        BUTTONS(19) = 'REFINE POLARISATION'
        TYPES(1) = Gs_real
        TYPES(2) = Gs_real
        TYPES(3) = Gs_real
        TYPES(4) = Gs_real
        ...
        TYPES(18) = Gs_logical
        TYPES(19) = Gs_logical
        TEXT(1) = 'SAMPLE TO DETECTOR DISTANCE (MM)'
        TEXT(2) = 'DETECTOR OFFSET FROM ROTATION AXIS (MM)'
        TEXT(3) = 'X COORDINATE FOR BEAM CENTRE (PIXELS)'
        TEXT(4) = 'Y COORDINATE FOR BEAM CENTRE (PIXELS)'
        TEXT(5) = 'WAVELENGTH (ANGSTROMS) (STARTING)'
        TEXT(6) = 'SIZE OF HORIZONTAL PIXELS (MICRONS)'
        TEXT(7) = 'SIZE OF VERTICAL PIXELS (MICRONS)'
        TEXT(8) = 'NUMBER OF AZIMUTHAL SECTIONS'
        TEXT(9) = 'REJECT OUT-LYING POSITIONS AND RE-REFINE'
        TEXT(10) = 'REJECT LIMIT FROM IDEAL (STANDARD DEVIATIONS)'
        TEXT(11) = 'OUTPUT FULL INFORMATION'
        TEXT(12) = 'REFINE DETECTOR OFFSET FROM ROTATION TO START'
        TEXT(13) = 'REFINE BEAM CENTRE'
        TEXT(14) = 'REFINE SAMPLE TO DETECTOR DISTANCE'
        TEXT(15) = 'REFINE X-RAY WAVELENGTH'
        TEXT(16) = 'REFINE DETECTOR NON-ORTHOGONALITY'
        TEXT(17) = 'FIT INTERMEDIATE NUMBER OF RINGS'
        TEXT(18) = 'CORRECT PARALLAX EFFECT ON ANGULAR POSITION'
        TEXT(19) = 'REFINE VALUE OF POLARISATION FACTOR'
        FULL_PROMPTS(1) = 'Enter approximate sample to ' // &
          'detector distance (mm)'
        FULL_PROMPTS(2) = 'Enter offset distance from rotation axis to ' // &
          'start of detector (mm)'
        FULL_PROMPTS(3) = 'Enter X-coordinate of beam centre in pixels '
        FULL_PROMPTS(4) = 'Enter Y-coordinate of beam centre in pixels '
        FULL_PROMPTS(5) = 'Enter approximate X-ray ' // &
          'wavelength (Angstroms)'
        FULL_PROMPTS(6) = 'Enter dimension of pixels ' // &
          'horizontally, as displayed (mm)'
        FULL_PROMPTS(7) = 'Enter dimension of pixels ' // &
          'vertically, as displayed (mm)'
        FULL_PROMPTS(8) = 'Enter number of azimuthal ' // &
          'divisions to find average ring positions'
        FULL_PROMPTS(9) = 'Enter "YES" to reject badly fitting' // &
          'positions and re-fit without them'
        FULL_PROMPTS(10) = 'Enter number of standard deviations' // &
          ' from ideal before rejection'
        FULL_PROMPTS(11) = 'Enter "YES" to output full ' // &
          'diagnostics information in terminal window'
        FULL_PROMPTS(12) = 'Refine distance from rotation axis to start ' // &
          'of detector'
        FULL_PROMPTS(13) = 'Refine beam centre (relative to rotation axis)'
        FULL_PROMPTS(14) = 'Refine sample to detector distance'
        FULL_PROMPTS(15) = 'Refine x-ray wavelength'
        FULL_PROMPTS(16) = 'Refine detector non-orthogonality'
        FULL_PROMPTS(17) = 'Enter "YES" to add extra partial' // &
          'fitting iterations for difficult problems'
        FULL_PROMPTS(18) = 'Enter "YES" to account for the effect of ' // &
          'parallax on angular position'
        FULL_PROMPTS(19) = 'Enter "YES" to refine value' // &
        ...
        REALS_LOWER(3) = -10000.0
        REALS_UPPER(3) = 10000.0
        REALS_LOWER(4) = -10000.0
        REALS_UPPER(4) = 10000.0
        REALS_LOWER(5) = 0.0001
        ...
        INTS_UPPER(8) = 360
        REALS_LOWER(10) = 2.0
        REALS_UPPER(10) = 100.0
 
        REALS(1) = experiment%detector_distance * 1000.0
        REALS(2) = experiment%detector_offset * 1000.0
        REALS(3) = experiment%x_beam
        REALS(4) = experiment%y_beam
        REALS(5) = experiment%wavelength * 1.0e10
        REALS(6) = experiment%x_pixel_size * 1.0e6
        REALS(7) = experiment%y_pixel_size * 1.0e6
        INTEGERS(8) = num_sections
        LOGICALS(9) = reject_outliers
        REALS(10) = reject_value
        LOGICALS(11) = full_info
        LOGICALS(12) = refine_detector_offset
        LOGICALS(13) = refine_beam_centre
        LOGICALS(14) = refine_detector_distance
        LOGICALS(15) = refine_wavelength
        LOGICALS(16) = refine_tilt
        LOGICALS(17) = extra_fits
        LOGICALS(18) = correct_parallax
        LOGICALS(19) = refine_polarisation
...
!     Set resulting values
        ...
        experiment%detector_offset = REALS(2) / 1000.0
        experiment%x_beam = REALS(3)
        experiment%y_beam = REALS(4)
        experiment%wavelength = REALS(5) / 1.0e10
        experiment%x_pixel_size =  REALS(6) / 1.0e6
        experiment%y_pixel_size =  REALS(7) / 1.0e6
        num_sections =  INTEGERS(8)
        reject_outliers = LOGICALS(9)
        reject_value = REALS(10)
        full_info = LOGICALS(11)
        refine_detector_offset = LOGICALS(12)
        refine_beam_centre = LOGICALS(13)
        refine_detector_distance = LOGICALS(14)
        refine_wavelength = LOGICALS(15)
        refine_tilt = LOGICALS(16)
        extra_fits = LOGICALS(17)
        correct_parallax = LOGICALS(18)
        refine_polarisation = LOGICALS(19)
...
!     Save parameter values in the data-base
        ...
        Call IO_SET_LKEYVALUE ('CALI_REFINE_OFFSET', refine_detector_offset, &
          retstat, status)
        Call IO_SET_LKEYVALUE ('CALI_REFINE_BEAM', refine_beam_centre, &
          retstat, status)
        Call IO_SET_LKEYVALUE ('CALI_REFINE_DISTANCE', refine_detector_distance,&
          retstat, status)
...
! - - - -1- - - - -2- - - - -3- - - - -4- - - - -5- - - - -6- - - - -7- - - - -8
!     Find bragg lines and refine detector parameters
        Call F2D_ID06_CALIBRANT4 (xmaxdat, ymaxdat, xnumdat, ynumdat, &
          xstrelm, ystrelm, xendelm, yendelm, XAXIS, YAXIS, DATA, MASK, &
          title, xlabel, ylabel, zlabel, azimuth_start, azimuth_end, &
          calibrant_sample, Max_angles, num_cali_rings, D_SPACINGS, &
...
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
!     Write (*, '(''F2D_ID06_CALIBRANT3: End'')')
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG

     End Subroutine F2D_ID06_CALIBRANT3
!********1*********2*********3*********4*********5*********6*********7*********8
