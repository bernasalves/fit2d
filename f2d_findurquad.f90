...
!+ F2D_FINDURQUAD - Fit2D: FIND Upper right QUADrant of peak positions
...
!      quadrant (Hammersley)
!    31-Oct-1993: V0.1 Original (Hammersley)
!  Type Definitions:
     Implicit None
!  Global Constants:
...
!      with grid peaks
     ...
     Integer, Intent(IN) :: xnumtemplate ! Number of elements in first dimension
!      in "TEMPLATE"
...
!  Local Variables:
     ...
     Integer :: xprevious ! Array element for X-direction for previously 
!      defined peak
     Integer :: y_subpixel ! Y-sub-pixel coordinate for peak centre
     Integer :: ypeak ! Array element for Y-direction
     Integer :: yprevious ! Array element for Y-direction for previously 
!      defined peak
...
!  Convert sub-pixel number to pixel coordinate
     Subpixel2pc(x_subpixel) = Real(x_subpixel) / Real(sub_pixelling) - &
       1.0 / Real(sub_pixelling * 2)
 
!--------1---------2---------3---------4---------5---------6---------7---------8
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
!  Write (*, '(''Entered F2D_FINDURQUAD'')')
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
 
!  Check status.
     If (status .Ne. St_goodvalue) Then
        Call ST_SAVE ('Subroutine F2D_FINDURQUAD ' // Version)
        Return
     End If
...
!        Calculate estimated position of peak from previously found peaks
           If (X_2DPEAKS(xpeak-1, ypeak) .Gt. -1.7e38 .And. X_2DPEAKS(xpeak-1, &
             ypeak - 1) .Gt. -1.7e38 .And. X_2DPEAKS(xpeak, ypeak - 1) .Gt. &
             -1.7e38) Then
 
!           The three corner peaks have been found, so vector
!           can be calculated
              x_coordinate = X_2DPEAKS(xpeak - 1, ypeak) + (X_2DPEAKS(xpeak, &
                ypeak-1) - X_2DPEAKS(xpeak - 1, ypeak - 1))
              y_coordinate = Y_2DPEAKS(xpeak - 1, ypeak) + (Y_2DPEAKS(xpeak, &
                ypeak-1) - Y_2DPEAKS(xpeak - 1, ypeak - 1))
...
!           Set-up for new row
              ...
              If (X_2DPEAKS(2, ypeak-1) .Gt. -1.7e38 .And. X_2DPEAKS(1, &
                ypeak - 1) .Gt. -1.7e38 .And. &
                X_2DPEAKS(1, ypeak) .Gt. -1.7e38) Then
...
!           Check coordinate is within search area
              continue = x_coordinate .Ge. Real(xstrelm-1) .And. x_coordinate &
                ...
                .And. y_coordinate .Le. Real(yendelm) .And. ypeak .Le. num_up
