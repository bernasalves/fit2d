...
!+ F2D_CALIBRANT -  CALIBRANT fit of wavelength / distance / etc.
...
!  History:
!    17-Apr-2013: V0.31 Option to take account of parallax (Hammersley)
!    20-Mar-2013: V0.30 Add Silver Behenate to list of calibrants (Hammersley)
!    15-Mar-2006: V0.29 Debug changes to beam centre definition (Hammersley)
!    10-Mar-2006: V0.28 Use "EXPERIMENTAL_DETAILS" structure (Hammersley)
!    05-Apr-2005: V0.27 Reduce minimum wavelength to 1.0e-4 Angstroms
!      (Hammersley)
!    12-Dec-2003: V0.26 Update graphics after "?" or "HELP" commands
!      (Hammersley)
!    24-Jul-2001: V0.25 Add possibility for user defined calibrant (Hammersley)
!    13-Apr-1999: V0.24 Add Al_2O_3 to available list of calibrants (Hammersley)
!    23-Feb-1999: V0.23 All data-base saving and recovering routines
!      renamed (Hammersley)
!    15-Dec-1998: V0.22 Change to use IO internal database routines (Hammersley)
!    30-Jan-1998: V0.21 Declare unused character string for call to
!      "GS_FORM" (Hammersley)
!    10-Jan-1998: V0.20 Add extra comma in format statement to please
!      AIX compiler (Hammersley)
!    13-Oct-1997: V0.19 Increase number of calibrant choices so that
!      "SILICON" is available again (Hammersley)
!    01-Oct-1997: V0.18 Obtain default values from database if
!      available and save to database (Hammersley)
!    30-Sep-1997: V0.17 Output beam centre in millimetres, and output
!      equivalent "ROT X" and "ROT Y" angles (Hammersley)
!    26-Sep-1997: V0.16 Reduce search limit for paraffin wax to
!      avoid search limit including second ring (Hammersley)
!    25-Sep-1997: V0.15 Add "NaCl (Salt)" to choice of calibrants (Hammersley)
!    25-Sep-1997: V0.14 Add "PARAFFIN WAX" to choice of calibrants (Hammersley)
!    08-Jul-1997: V0.13 Add "F2D_AZIMUTHS" array for "F2D_FINDRINGS", 
!      and option to refine the polarisation factor (Hammersley)
!    27-Feb-1997: V0.12 Make user output text correct for non-silicon 
!      calibrant cases (Hammersley)
!    25-Feb-1997: V0.11 Add support for Cerium Dioxide and Lanthanum 
!      Hexaboride (Hammersley)
!    24-Feb-1997: V0.10 Rename from "F2D_SILICON" (Hammersley)
!    21-Feb-1997: V0.9 Option of extra fitting stages (Hammersley)
!    16-Dec-1996: V0.8 Avoid open strings crossing lines (Hammersley)
!    26-Aug-1996: V0.7 Reduce search region for second search, fit on 4 rather 
!      than 3 initial rings, output d-spacing related statistic for stability 
!      purposes, allow distance to be fixed (Hammersley)
!    24-Aug-1996: V0.6 Return wavelength (Hammersley)
!    23-Aug-1996: V0.5 Changes to "GS_INPS_COORDINATES" (Hammersley)
!    19-Aug-1996: V0.4 Check that initial estimates of distance and
!      wavelength are consistent with radius of Si 111 ring (Hammersley)
!    02-Jul-1996: V0.3 Save beam centre, and use, if not being refined
!      (Hammersley)
!    14-Jun-1996: V0.2 Add options to refine or not all parameters (Hammersley)
!    10-Jun-1996: V0.1 Original (Hammersley)
!  Type Definitions:
     Implicit None
     Include 'io.inc'
!  Global Constants:
...
!  Check input status
     If (status .Ne. St_goodvalue) Then
        Call ST_SAVE ('Subroutine F2D_CALIBRANT ' // Version)
        Return
     End If
...
!     choose number of output bins and control various other options
        num_choices = 14 !!!! DISABLE "REFINE POLARISATION" AT PRESENT
        PROMPT(1) = 'CALIBRANT PATTERN REFINEMENT'
        PROMPT(2) = 'OF DISTANCE WAVELENGTH ETC.'
        BUTTONS(1) = 'DISTANCE'
        BUTTONS(2) = 'WAVELENGTH'
        BUTTONS(3) = 'X-PIXEL SIZE'
        BUTTONS(4) = 'Y-PIXEL SIZE'
        BUTTONS(5) = 'ANGULAR SECTIONS'
        BUTTONS(6) = 'REJECT OUTLIERS'
        BUTTONS(7) = 'REJECT LIMIT'
        BUTTONS(8) = 'FULL INFO'
        BUTTONS(9) = 'REFINE BEAM X/Y'
        BUTTONS(10) = 'REFINE DISTANCE'
        BUTTONS(11) = 'REFINE WAVELENGTH'
        BUTTONS(12) = 'REFINE TILT'
        BUTTONS(13) = 'EXTRA ITERATIONS'
        BUTTONS(14) = 'CORRECT PARALLAX'
        BUTTONS(15) = 'REFINE POLARISATION'
        TYPES(1) = Gs_real
        TYPES(2) = Gs_real
        TYPES(3) = Gs_real
        TYPES(4) = Gs_real
        ...
        TEXT(2) = 'WAVELENGTH (ANGSTROMS) (STARTING)'
        TEXT(3) = 'SIZE OF HORIZONTAL PIXELS (MICRONS)'
        TEXT(4) = 'SIZE OF VERTICAL PIXELS (MICRONS)'
        TEXT(5) = 'NUMBER OF AZIMUTHAL SECTIONS'
        TEXT(6) = 'REJECT OUT-LYING POSITIONS AND RE-REFINE'
        TEXT(7) = 'REJECT LIMIT FROM IDEAL (STANDARD DEVIATIONS)'
        TEXT(8) = 'OUTPUT FULL INFORMATION'
        TEXT(9) = 'REFINE X/Y BEAM CENTRE'
        TEXT(10) = 'REFINE SAMPLE TO DETECTOR DISTANCE'
        TEXT(11) = 'REFINE X-RAY WAVELENGTH'
        TEXT(12) = 'REFINE DETECTOR NON-ORTHOGONALITY'
        TEXT(13) = 'FIT INTERMEDIATE NUMBER OF RINGS'
        TEXT(14) = 'CORRECT PARALLAX EFFECT ON ANGULAR POSITION'
        TEXT(15) = 'REFINE VALUE OF POLARISATION FACTOR'
        FULL_PROMPTS(1) = 'Enter approximate sample to ' // &
          'detector distance (mm)'
        FULL_PROMPTS(2) = 'Enter approximate X-ray ' // &
          'wavelength (Angstroms)'
        FULL_PROMPTS(3) = 'Enter dimension of pixels ' // &
          'horizontally, as displayed (mm)'
        FULL_PROMPTS(4) = 'Enter dimension of pixels ' // &
          'vertically, as displayed (mm)'
        FULL_PROMPTS(5) = 'Enter number of azimuthal ' // &
          'divisions to find average ring positions'
        FULL_PROMPTS(6) = 'Enter "YES" to reject badly fitting' // &
          'positions and re-fit without them'
        FULL_PROMPTS(7) = 'Enter number of standard deviations' // &
          ' from ideal before rejection'
        FULL_PROMPTS(8) = 'Enter "YES" to output full ' // &
          'diagnostics information in terminal window'
        FULL_PROMPTS(9) = 'Refine x/y beam centre'
        FULL_PROMPTS(10) = 'Refine sample to detector distance'
        FULL_PROMPTS(11) = 'Refine x-ray wavelength'
        FULL_PROMPTS(12) = 'Refine detector non-orthogonality'
        FULL_PROMPTS(13) = 'Enter "YES" to add extra partial' // &
          'fitting iterations for difficult problems'
        FULL_PROMPTS(14) = 'Enter "YES" to account for the effect of ' // &
          'parallax on angular position'
        FULL_PROMPTS(15) = 'Enter "YES" to refine value' // &
        ...
        REALS(2) = experiment%wavelength * 1.0e10
        REALS(3) = experiment%x_pixel_size * 1.0e6
        REALS(4) = experiment%y_pixel_size * 1.0e6
        INTEGERS(5) = num_sections
        LOGICALS(6) = reject_outliers
        REALS(7) = reject_value
        LOGICALS(8) = full_info
        LOGICALS(9) = refine_beam_centre
        LOGICALS(10) = refine_sample_distance
        LOGICALS(11) = refine_wavelength
        LOGICALS(12) = refine_tilt
        LOGICALS(13) = extra_fits
        LOGICALS(14) = correct_parallax
        LOGICALS(15) = refine_polarisation
...
!     Set resulting values
        ...
        experiment%wavelength = REALS(2) / 1.0e10
        experiment%x_pixel_size =  REALS(3) / 1.0e6
        experiment%y_pixel_size =  REALS(4) / 1.0e6
        num_sections =  INTEGERS(5)
        reject_outliers = LOGICALS(6)
        reject_value = REALS(7)
        full_info = LOGICALS(8)
        refine_beam_centre = LOGICALS(9)
        refine_sample_distance = LOGICALS(10)
        refine_wavelength = LOGICALS(11)
        refine_tilt = LOGICALS(12)
        extra_fits = LOGICALS(13)
        correct_parallax = LOGICALS(14)
        refine_polarisation = LOGICALS(15)
...
!     Save beam centre, if not to be refined
        If (.Not. refine_beam_centre) Then
           x_ellipse = experiment%x_beam * experiment%x_pixel_size
           y_ellipse = experiment%y_beam * experiment%y_pixel_size
        End If
...
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
!        Write (*, '(''F2D_CALIBRANT: After GS_2DIMAGE'')')
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
...
!     Input three or more coordinates
        ...
        If (calibrant_sample .Eq. 'ALUMINA (Al2O3') Then
           MESSAGE(1) = 'Click on three or more coordinates on the 1 0 -2'
           MESSAGE(2) = 'reflection ring (the closest to the centre).'
        Else If (calibrant_sample .Eq. 'LANTHANUM HEXABORIDE') Then
           MESSAGE(1) = 'Click on three or more coordinates on the 100'
           MESSAGE(2) = 'reflection ring (the closest to the centre).'
        Else If (calibrant_sample .Eq. 'PARAFFIN WAX') Then
           MESSAGE(1) = 'Click on three or more coordinates on the first'
           MESSAGE(2) = 'reflection ring (the closest to the centre).'
        Else If (calibrant_sample .Eq. 'SODIUM CHLORIDE') Then
           MESSAGE(1) = 'Click on three or more coordinates on the 200'
           MESSAGE(2) = 'reflection ring (the second from the centre).'
        Else If (calibrant_sample .Eq. 'SILVER BEHENATE') Then
           MESSAGE(1) = 'Click on three or more coordinates on the 001'
           MESSAGE(2) = 'reflection ring (the first from the centre).'
        Else
           MESSAGE(1) = 'Click on three or more coordinates on the 111'
           MESSAGE(2) = 'reflection ring (the closest to the centre).'
        End If
        MESSAGE(3) = 'If 3 or 4 coordinates are entered a circle'
        MESSAGE(4) = 'will fitted, otherwise an ellipse will be'
        MESSAGE(5) = 'fitted. If there are more coordinates than'
        MESSAGE(6) = 'fitted variables, then an estimate of the'
        MESSAGE(7) = 'positional error will be produced.'
 
        If (calibrant_sample .Eq. 'SODIUM CHLORIDE') Then
           user_prompt = 'CLICK ON THE 200 (SECOND) SAMPLE RING (>=3)'
        Else
           user_prompt = 'INPUT COORDINATES ON INNER SAMPLE RING (>=3)'
        End If
 
        Call GS_INPS_FCOORDINATES (.True., .True., xmaxdat, ymaxdat, xstrelm, &
          ystrelm, xendelm, yendelm, DATA, MASK, XAXIS, YAXIS, title, xlabel, &
          ylabel, zlabel, user_prompt, 7, MESSAGE, .False., Max_coordinates, &
          num_coordinates, X_COORDINATES, Y_COORDINATES, status)
 
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
!        Write (*, '(''F2D_CALIBRANT: After GS_INPS_FCOORDINATES'')')
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
...
!        Convert to metre units
           ...
           If (num_coordinates .Ge. 5) Then
 
!           Fit ellipse
              Call F2D_LSQELLIPSE (Max_coordinates, num_coordinates, &
                X_COORDINATES, Y_COORDINATES, x_ellipse, y_ellipse, radius1, &
...
!           Fit circle
           ...
           experiment%y_beam = y_ellipse / experiment%y_pixel_size
 
!        Output information to user
           Write (MESSAGE(1), '(''INFO: Best fit ' // &
             'centre X / Y (mm) = '', 2g14.7)') &
             x_ellipse * 1000.0, y_ellipse * 1000.0
           Call IO_WRITE (MESSAGE(1), status)
           Write (MESSAGE(1), '(''INFO: Best fit ' // &
             'centre X / Y (pixels) = '', 2g14.7)') &
             experiment%x_beam, experiment%y_beam
           Call IO_WRITE (MESSAGE(1), status)
           Write (MESSAGE(1), '(''INFO: Best fit ' // &
             'radius 1, radius 2 (mm) = '', 2g14.7)') radius1 * 1000.0, &
             radius2 * 1000.0
           Call IO_WRITE (MESSAGE(1), status)
           If (num_coordinates .Lt. 5) Then
              Write (MESSAGE(1), '(''INFO: Best fit ' // &
                'radius (X-pixels) = '', g14.7)') &
                radius2 / experiment%x_pixel_size
              Call IO_WRITE (MESSAGE(1), status)
           Else
              Write (MESSAGE(1), '(''INFO: Best fit ' // &
                'radius 1, radius 2 (X-pixels) = '', 2g14.7)') radius1 / &
                experiment%x_pixel_size, radius2 / experiment%x_pixel_size
              Call IO_WRITE (MESSAGE(1), status)
              Write (MESSAGE(1), '(''INFO: Best fit angle of ' // &
                'axis 1 (degrees) = '', g14.7)') angle * 180.0 / Pi
              Call IO_WRITE (MESSAGE(1), status)
           End If
 
           If (num_coordinates .Eq. 4 .Or. num_coordinates .Gt. 5) Then
              Write (MESSAGE(1), '(''INFO: Estimated coordinate ' // &
                'radial position error (mm) = '', g14.7)') radial_error * &
                1000.0
              Call IO_WRITE (MESSAGE(1), status)
              Write (MESSAGE(1), '(''INFO: Estimated coordinate ' // &
                'radial position error (X pixels) = '', g14.7)') &
...
!     Estimate for half-search distance
        half_search_distance = Max(radius1, radius2) / 8.0
        F2D_RADIA(1) = (radius1 + radius2) / 2.0
 
!     Reduce search annulus for wax, as previously this included the second ring
        If (calibrant_sample .Eq. 'PARAFFIN WAX' .Or. calibrant_sample .Eq. &
          'SODIUM CHLORIDE') Then
           half_search_distance = half_search_distance / 2.0
        End If
 
!     Draw red ellipse on top of graphics
        Call F2D_ELLIPSE (experiment%x_pixel_size, experiment%y_pixel_size, &
          x_ellipse, y_ellipse, radius1, radius2, angle, Gs_red, status)
 
!     Draw white limit ellipses on top of graphics
        Call F2D_ELLIPSE (experiment%x_pixel_size, experiment%y_pixel_size, &
          x_ellipse, y_ellipse, radius1 - half_search_distance, &
          radius2 - half_search_distance, angle, Gs_white, status)
        Call F2D_ELLIPSE (experiment%x_pixel_size, experiment%y_pixel_size, &
          x_ellipse, y_ellipse, radius1 + half_search_distance, &
          radius2 + half_search_distance, angle, Gs_white, status)

!     Output information prompt
        Call GS_FPROMPT (1, 1, 'CALCULATING COORDINATES ON INNER RING', &
          status)
...
! - - - -1- - - - -2- - - - -3- - - - -4- - - - -5- - - - -6- - - - -7- - - - -8
!     Use fitted ellipse parameters to calculate centre of gravity in segments
!     around the initially input powder ring
        Call F2D_FINDRINGS (full_info, xmaxdat, ymaxdat, xstrelm, ystrelm, &
          xendelm, yendelm, DATA, MASK, experiment, &
          F2d_max_rings, 1, F2D_RADIA, half_search_distance, &
...
!     Output number of accepted coordinates
        If (full_info) Then
           Write (MESSAGE(1), '(''INFO: Number of ' // &
             'acceptable coordinates on ring = '', i3)') &
...
!     the beam centre, radius, etc.
        ...
        If (num_coordinates .Ge. 3) Then
           ...
           Call F2D_LSQELLIPSE (F2d_max_rcoordinates, F2D_NUM_RCOORDINATES(1), &
             ...
             radius2, angle, radial_error, status)
        Else
           Call IO_WRITE ('WARNING: No coordinates found on ring', status)
        End If
...
!     Calculate ideal distance for detector given the wavelength
        two_theta = 2.0 * Asin (experiment%wavelength / (2.0 * D_SPACINGS(1)))
        est_distance = Min(radius1, radius2) / Tan(two_theta)
        If (full_info) Then
           Write (MESSAGE(1), '(''INFO: Estimated ' // &
             'sample to detector distance = '', f12.3, '' mm'')') est_distance &
             * 1000.0
           Call IO_WRITE (MESSAGE(1), status)
        End If

!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
!        Write (*, '(''two_theta = '', f12.5)') two_theta
!        Write (*, '(''D_SPACINGS(1) = '', e12.5)') D_SPACINGS(1)
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG

!     Check that estimated distance is similar to given distance
        If (est_distance / experiment%detector_distance .Lt. 0.8 .Or. &
          est_distance / experiment%detector_distance .Gt. 1.2) Then
 
!        Output warning message
           Call IO_WRITE ('WARNING: The calculated sample ' // &
             'to detector distance (given the initial', status)
           Call IO_WRITE ('         estimate of the ' // &
             'wavelength)  is  too different from  the user', status)
           Call IO_WRITE ('   estimate of  the ' // &
             'wavelength.  Please check the initial values', status)
           Call IO_WRITE ('         of  both the distance ' // &
             'and the wavelength and alter them using', status)
           Call IO_WRITE ('         the graphical entry ' // &
             'form. Also check that you are using the', status)
           Call IO_WRITE ('         correct ring.', status)
 
           Call GS_FWARNING (1, 1, 'WARNING: Too poor initial ' // &
             'values for distance and wavelength', status)
...
!        Immediate exit
        ...
        If (.Not. refine_sample_distance) Then
           est_distance = experiment%detector_distance
 
           If (refine_wavelength) Then
 
!           Set initial estimate of wavelength
              two_theta = Atan2(Min(radius1, radius2), &
                experiment%detector_distance)
              experiment%wavelength = 2.0 * D_SPACINGS(1) * Sin(two_theta / 2.0)
 
!           Output user information
              Write (MESSAGE(1), '(''INFO: Initial wavelength, ' // &
                'based on fixed distance = '', f8.5, '' Angstroms'')') &
...
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
!        Write (*, '(''refine_sample_distance = '', l1)') refine_sample_distance
!        Write (*, '(''detector_distance = '', f12.5)') &
!          experiment%detector_distance
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG

! - - - -1- - - - -2- - - - -3- - - - -4- - - - -5- - - - -6- - - - -7- - - - -8
!     Calculate radii for search of first 4 Bragg rings
        num_rings = Min(4, num_cali_rings)
        Do radii = 2, num_rings
           F2D_RADIA(radii) = est_distance * &
             Tan( 2.0 * Asin (experiment%wavelength / &
             (2.0 * D_SPACINGS(radii))))
 
!        Draw "ellipses" on detector
           Call F2D_ELLIPSE (experiment%x_pixel_size, experiment%y_pixel_size, &
             x_ellipse, y_ellipse, &
             F2D_RADIA(radii), F2D_RADIA(radii), angle, Gs_red, status)
 
!        Draw white limit ellipses on top of graphics
           Call F2D_ELLIPSE (experiment%x_pixel_size, experiment%y_pixel_size, &
             x_ellipse, y_ellipse, &
             F2D_RADIA(radii) - half_search_distance, F2D_RADIA(radii) - &
             half_search_distance, angle, Gs_white, status)
           Call F2D_ELLIPSE (experiment%x_pixel_size, experiment%y_pixel_size, &
             x_ellipse, y_ellipse, &
             F2D_RADIA(radii) + half_search_distance, F2D_RADIA(radii) + &
             half_search_distance, angle, Gs_white, status)
...
!     in segments around the required powder rings
        Call F2D_FINDRINGS (full_info, xmaxdat, ymaxdat, xstrelm, ystrelm, &
          xendelm, yendelm, DATA, MASK, experiment, &
          F2d_max_rings, num_rings, F2D_RADIA, half_search_distance, &
          num_sections, .False., F2d_max_rcoordinates, F2D_NUM_RCOORDINATES, &
          F2D_X_RCOORDINATES, F2D_Y_RCOORDINATES, F2D_RINTENSITIES, &
          F2D_AZIMUTHS, status)
...
!     Display centre of gravity coordinates on top of graphics
        Call F2D_TILTDISPLAY ( &
          ...
          F2D_NUM_RCOORDINATES, F2D_X_RCOORDINATES, F2D_Y_RCOORDINATES, status)
 
!     Refine wavelength, sample-detector distance, tilt to first four 
!     diffraction rings
        experiment%detector_distance = est_distance
        Call F2D_FITCALIBRANT (full_info, F2d_max_rings, num_rings, &
          D_SPACINGS, weighted_fitting, reject_outliers, &
          reject_value, refine_beam_centre, refine_sample_distance, &
          refine_wavelength, refine_tilt, experiment, &
          radial_error, status)
...
!     Reduce search area
        ...
        If (extra_fits .And. num_cali_rings .Gt. 4) Then
 
!        Fit 7 rings then 11 rings
 
!        Calculate radii for search of first 15 Bragg rings
           num_rings = Min(7, num_cali_rings)
           Do radii = 1, num_rings
              F2D_RADIA(radii) = experiment%detector_distance * &
                Tan( 2.0 * Asin (experiment%wavelength &
                / (2.0 * D_SPACINGS(radii))))
...
!        Refinement using 7 rings
 
!        Use fitted ellipse parameters to calculate centre of gravity
!        in segments around the required powder rings
           Call F2D_FINDRINGS (full_info, xmaxdat, ymaxdat, xstrelm, ystrelm, &
             xendelm, yendelm, DATA, MASK, experiment, &
             F2d_max_rings, num_rings, F2D_RADIA, &
...
!        Refine wavelength, sample-detector distance, tilt to first three 
!        diffraction rings
           Call F2D_FITCALIBRANT (full_info, F2d_max_rings, num_rings, &
             D_SPACINGS, weighted_fitting, reject_outliers, &
             reject_value, refine_beam_centre, refine_sample_distance, &
           ...
           If (num_cali_rings .Gt. 4) Then
 
!           Calculate radii for search of first 11 Bragg rings
              num_rings = Min(11, num_cali_rings)
              Do radii = 1, num_rings
                 F2D_RADIA(radii) = experiment%detector_distance * &
                   Tan( 2.0 * Asin &
                   (experiment%wavelength / (2.0 * D_SPACINGS(radii))))
...
!           Refinement using 11 rings
 
!           Use fitted ellipse parameters to calculate centre of
!           gravity in segments around the required powder rings
              Call F2D_FINDRINGS (full_info, xmaxdat, ymaxdat, xstrelm, &
                ystrelm, xendelm, yendelm, DATA, MASK, experiment, &
                F2d_max_rings, num_rings, &
                F2D_RADIA, half_search_distance, num_sections, .False., &
...
!           Refine wavelength, sample-detector distance, tilt to first
!           three diffraction rings
              Call F2D_FITCALIBRANT (full_info, F2d_max_rings, num_rings, &
                D_SPACINGS, weighted_fitting, reject_outliers, &
                reject_value, refine_beam_centre, refine_sample_distance, &
                refine_wavelength, refine_tilt, experiment, radial_error, &
                status)
...
! - - - -1- - - - -2- - - - -3- - - - -4- - - - -5- - - - -6- - - - -7- - - - -8
!        Refinement using 15 rings (if available)
...
!        Refine wavelength, sample-detector distance, tilt to diffraction rings
           Call F2D_FITCALIBRANT (full_info, F2d_max_rings, num_rings, &
             D_SPACINGS, weighted_fitting, reject_outliers, &
             reject_value, refine_beam_centre, refine_sample_distance, &
             refine_wavelength, refine_tilt, experiment, radial_error, &
...
!        Output results
           ...
           Write (MESSAGE(1), '(''INFO: ROT X = '', f12.3, ' // &
             ...
             * 180.0 / Pi
           Call IO_WRITE (MESSAGE(1), status)
!        Write (MESSAGE(1), '(''INFO: Stability indicator = '', ' //
!        :       'f10.5)') sample_distance * Tan(2.0 *

!        :       Asin (wavelength / (2.0 * D_SPACINGS(1))))
           Write (MESSAGE(1), '(''INFO: Stability indicator ' // &
             '(proportional to D-spacing; Angstroms) = '', f10.5)') &
             1.0e10 * experiment%wavelength / (2.0 * Sin(0.5 * Atan2(0.1, &
             experiment%detector_distance)))
           Call IO_WRITE (MESSAGE(1), status)
...
! - - - -1- - - - -2- - - - -3- - - - -4- - - - -5- - - - -6- - - - -7- - - - -8
!     Refine polarisation from integrated intensities of data points if
!     required
        If (refine_polarisation) Then
 
!        Output progress message
           Call GS_FPROMPT (1, 1, 'CALCULATING INTEGRATED INTENSITES', status)
 
!        Force output
           Call GS_UPDATE (status)
 
!        Find and calculate integrated intensities of data points
           Call F2D_FINDRINGS (full_info, xmaxdat, ymaxdat, xstrelm, ystrelm, &
             xendelm, yendelm, DATA, MASK, experiment, &
             F2d_max_rings, num_rings, F2D_RADIA, &
             half_search_distance, num_sections, .True., F2d_max_rcoordinates, &
             F2D_NUM_RCOORDINATES, F2D_X_RCOORDINATES, F2D_Y_RCOORDINATES, &
             F2D_RINTENSITIES, F2D_AZIMUTHS, status)
 
!        Refine polarisation factor from integrated intensities
           Call F2D_LSQPOLARISATION (experiment%detector_distance, &
             F2d_max_rings, &
             num_rings, F2d_max_rcoordinates, experiment%polarisation, status)
 
           Write (message, '(''INFO: Refined polarisation factor = '', f8.5)') &
...
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG
!     Write (*, '(''F2D_CALIBRANT: End'')')
!**DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG***DEBUG

     End Subroutine F2D_CALIBRANT
!********1*********2*********3*********4*********5*********6*********7*********8
